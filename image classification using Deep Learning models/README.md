...
The dataset consists of images categorized into two classes: cars and planes. Initially, I loaded and processed images from the specified directories, resizing them to 224x224 pixels, The images were normalized to enhance model training efficiency by scaling pixel values between 0 and 1, I split the data into training and validation sets, ensuring a reliable evaluation of the models performance, This process involved converting the images to numpy arrays and reshaping them when necessary to fit specific model architectures, This preprocessing aimed to ensure consistency across CNN, RNN, and FNN models. 
CNN Model Development 
The CNN model was developed to capture spatial hierarchies in the image data. The 
architecture included three convolutional layers, each followed by a max pooling 
layer, which helped reduce dimensionality and capture essential features. The final 
dense layer with a sigmoid activation function outputted binary classification results. 
...
models superior performance, with an Area Under the Curve (AUC) nearing 0.9, 
